...
/**
 * 锦标赛入口是否对用户可见(2026-09-24 拍板:锦标赛恢复默认隐藏)：既要 championship 活动自己的开关
 * 打开，也要活动页「显示锦标赛」开关(activityCenter.showChampionship)打开；后者默认 false，所以
 * 锦标赛在两处开关都保持默认时不可见——isOpenChampion 字段是"我的"页锦标赛入口、活动页赛事卡数据
 * 拉取共用的唯一开关，这里统一收口，不用在各个消费它的前端文件里分别判断。
 */
...
function activeSetting(ctx: MockContext) {
	...
	return {
		...
		showTaskStatusFilter: flag(params<{ showStatusFilter: boolean }>(ctx, 'dailyTask').showStatusFilter),
