<template>
	<div class="lottery_container">
		<div class="lotterySlotItemBig"
			 :data-key="`${item.class}${item.typeId}`"
			 :class="item.class" v-for="item in list[currentLottery]" :key="item.typeId" @click="isAlowGame(item, onItemClick)">
		</div>
	</div>
</template>
...
.lottery_container {
	...
	@each $key in $big-mumbai-items {
		.lotterySlotItemBig[data-key="#{$key}"] {
			background-image: url('@public/home/bigMumbai/#{$key}.png');
		}
	}
	//.wingo {
