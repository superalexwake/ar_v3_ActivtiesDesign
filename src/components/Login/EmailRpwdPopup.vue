<template>
	...
	<van-popup v-model:show="showPopup" :close-on-click-overlay="false" position="bottom" round>
		<div class="RpwdPopup">
			...
			<VerifyEmailInput v-model:value="formData.smsvcode" :sendFunc="sendSMS" :email="email" type-p="lock" />
			...
			<div class="gotuserver" @click="goSever">
				<svg-icon name="customer1" />{{ $t('contactServicer') }}
			</div>
			<div class="errorTip">{{ $t('wrongemail') }}<br />{{ $t('rpwdEmailPopupTip') }}</div>
