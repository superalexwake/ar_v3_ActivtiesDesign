<template>
	<div class="x-order pt10 pr15 pl15">
		...
		<div v-if="pageData.type === 0">
				...
				<div class="item">
					<span class="amount">
						...
						<div class="amount-copy">
							 <svg-icon
								 @click="copy(pageData.info.amount)"
								 name="copy"
							 />
			...
			<div v-if="handleToPayType!=='Paytm'">
				<div class="x-title">
					<span></span>
					<p>{{$t('choosePay')}}</p>
				</div>
				<div class="x-order-pay x-row x-flex-warp">
					<div class="item" :class="{active:handleToPayType===item.name,[item.name]:true}" v-for="(item, index) in payList" :key="index" @click="item.name==='Paytm'?handleToPaytmmp():handleToPay(item)">
			...
			<div :class="{order_active:handleToPayType==='PhonePe'}" >
						...
						<div>
							...
							<div class="utr-copy" v-if="showCopy">
								...
								<svg-icon
									...
									@click="copy(pageData.info.upiId)"
									name="copy"
									class="copy-icon"
								/>
							</div>
							<h3 class="x-tc mt8">1.{{$t('scanPayTip')}}</h3>
							<p class="x-tc mt8 mb16">
								2.{{$t('deviceQR')}}.
							</p>
						</div>
					</div>
				...
				<div class="x-order-utr">
					<p>{{$t('fillUTR')}}.</p>
					<div class="box">
						<van-field
							v-model="utrVal"
							:maxlength="12"
							:disabled="isUtr ? true : false"
							type="digit"
							autocomplete="new-password"
							:placeholder="$t('inputDigits')"
							class="inp"
						>
							<template #button>
								<van-button size="small" type="primary" @click="onPaste">{{$t('paste')}}</van-button>
							</template>
						</van-field>
						<!-- <div class="copy" @click="submit">Submit</div> -->
					</div>
				</div>
			</div>
		...
		<div class="x-order-food" v-if="[0,4].includes(pageData.type) && !isUtr">
			...
			>
				{{ pageData.type==4?$t('withdrawDialogDesc6'):$t('cancel') }}
			</div>
			<div class="submit" :class="isUtr ? 'active' : utrVal?'active':''" @click="submit">{{ $t('submit') }} {{utrVal?'':$t('unSubmitUtr')}}</div>
		</div>
		<div v-if="pageData.type == 1" class="x-row x-column x-row-middle x-tc expired">
			...
			<van-button round block type="primary" class="x-btn btn mt40" @click="onClick"> {{$t('returnMerchant')}} </van-button>
		</div>
		<div v-if="pageData.type == 3" class="x-row x-column x-row-middle x-tc submit-complete">
			<svg xmlns="http://www.w3.org/2000/svg" width="72" height="72" viewBox="0 0 72 72" fill="none">
				<path fill-rule="evenodd" clip-rule="evenodd" d="M36 6.96774C19.9659 6.96774 6.96774 19.9659 6.96774 36C6.96774 52.0341 19.9659 65.0323 36 65.0323C52.0341 65.0323 65.0323 52.0341 65.0323 36C65.0323 19.9659 52.0341 6.96774 36 6.96774ZM0 36C0 16.1177 16.1177 0 36 0C55.8822 0 72 16.1177 72 36C72 55.8822 55.8822 72 36 72C16.1177 72 0 55.8822 0 36Z" fill="#F6B02C"/>
		...
		<van-dialog class="arupi-dialog" :closeOnClickOverlay="false"  v-model:show="arupiTips" :show-confirm-button="false" :width="327">
			<div class="arupi">
		...
		<van-dialog class="arupi-dialog" :closeOnClickOverlay="false"  v-model:show="confirmShow" :show-confirm-button="false" :width="327">
			<div class="arupi">
				<div class="title1">{{ $t('cancelDeal') }}</div>
				<div class="title2">{{$t('paymeConfirm')}}</div>
				<div class="button">
					<div class="clance" @click="
					() => {
						from.checked = -1
						from.text = ''
						show = true;
				...
				">{{ $t('confirmCancel') }}</div>
					<div class="goBuy"  @click="handleToInput()">{{ $t('payme') }}</div>
				</div>
			</div>
		</van-dialog>
...
import { formatUpiTime, getFormatAmount,isOpenExternalUrl,openExternalUrl, copy } from '@/utils'
...
import { paymentDetails, confirmPayment, cancelPayment, cancellationReasonList, subForWakeUp } from '@/api'
import { Pay } from '@/api/arupi/types'
import { aesDecryptECB } from './crypto'
...
const getInfo = async () => {
	...
	if (code === '1') {
		...
		console.log('Decrypted payment details:', source);
		...
		await nextTick()
		text.value = 'upi://pay?pa=' + source.upiId + '&am=' + realAmount.value + '&cu=INR'+"&tn=" + source.tid + "&notes=" + source.tid + "&transactionDescription=" + source.tid+"&tid=" + source.tid + "&tr=" + source.tid;
...
function testPhonePeNativeDeeplink() {
	...
	try {
		//  console.log('Testing PhonePe Native Deeplink for ₹100');
		// 创建₹100的PhonePe深度链接数据
		const deeplinkData = {
			"contact": {
				"type": "VPA",
				"vpa": upiId,
		...
		console.log('Attempting to open PhonePe Deeplink for ₹' + deeplinkUrl)
...
const handleToPay = (item: Record<string, any>) => {
	...
	if (item.url.startsWith('paytmmp://cash_wallet')) {
		url += `?featuretype=money_transfer&pa=${pageData.info.upiId}&am=${realAmount.value}&cu=INR&tn=${pageData.info?.tid||''}`////
...
const submit = useThrottleFn(async () => {
	if (isUtr.value) return
	if (!utrVal.value || utrVal.value.length < 12) {
		return showFailToast(t('formatIncorrect'))
	}
	const res = await confirmPayment({ utr: utrVal.value })
	if (res.code === '1') {
		pageData.type = 3;
