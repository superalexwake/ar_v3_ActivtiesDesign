<template>
	<div class="x-appeal">
		<NavBar :title="t('buyAppeal')"  :placeholder="false" left-arrow @click-left="toBack" >
			<template #right>
		...
		<div class="x-appeal-box" v-if="pageData.pageId == 1&&!existAppealUtr">
			...
			<div v-else>
				...
				<div class="sub-enter" @click="router.push({name:'Appeal'})">{{$t('otherWllets')}}</div>
		...
		<div class="x-appeal-box" v-else-if="pageData.pageId !==1&&!existAppealUtr">
			...
			<KycUploadImage v-model:fileListImg="fileListImg"  />
			<div class="item mb16">
...
import KycUploadImage from "@/components/ArUPI/KycUploadImage.vue";
...
import { KycSendMsg, GetAppealKycBankList, KycVerify ,RechargeAppealExist } from "@/api";
...
const onRegister = useThrottleFn(async() =>{
	...
	if (fromData.bankCode==='airtel'){
		...
		try {
			const resultImage = await codeReader.decodeFromImageUrl(updateImg.value);
...
const onSubmit = async() =>{
	const { mobileNumber,bankCode , verificationCode ,utrVal,upiId} = fromData;
	let imagesList = fileListImg.value.map((item: { url: string; file: { objectName: any; }; }) => {
		if (item.file && item.file.objectName) {
			return item.file.objectName; // Get the filename from the objectName property
		}
	});
	console.log(imagesList,fileListImg,'fileListImg.value');
	const res = await KycVerify({ upiId,bankCode,otp:verificationCode,phoneNumber:mobileNumber as string,token,type:1,response:smgFun.value.kycMsg,utr:utrVal, images: imagesList.join(',')});
	if(res.code === '1'){
