<template>
	...
	<div class="WinningContainer">{{ $t('homename1') }}</div>
	<div class="Winningdata">
...
const {isSassLotteryGame ,openThirdGame,showChanglong,goChangLong} = useHome()
const lotteryData = computed(() => gameData.Lottery)

const lotteryMap :any  = {
	1: {
	...
	9: {
		title: t('moto8'),
		describe: t('moto9'),
		RouterName: 'AllLotteryGames-MotoRace'
	}
}

const goLottery = (id: any) => {
	const info:any=lotteryData.value.find((item)=>id==item.id);
	if(isSassLotteryGame(info)){
		return openThirdGame({...info,vendorCode:'ARLottery'})
	}
	let query
	if (id == 6 || id == 5) {
		query = {
...
const goLotteryNew = (name: string) => {
	router.push({name})
}
...
@import '@/assets/styles/Lottery.scss';
</style>
