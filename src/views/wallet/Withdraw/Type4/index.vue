<template>
	<div class="type4_C">
		...
		<div class="type4_C-addbtn" v-throttle-click="{ handler: AddUPI, wait: 1000 }">{{ $t('upiAddPaymentMethod') }}</div>
	</div>
</template>
...
const onBack = () => {
	...
	router.replace({ name: 'Withdraw', query: { bid, type: withdrawType } })
}
 ...
 */
const AddUPI = async () => {
	router.replace({ name: 'Withdraw-AddType4', query: { Type4name: Type4name, withdrawType: withdrawType } })
}
 ...
 */
const onLoad = async () => {
	const res = await AwaitApiResult(GetWithdrawals({ withdrawid: withdrawType }))
	if (res) {
