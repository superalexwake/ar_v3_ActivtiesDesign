			...
			<template v-if="upiList.length">
				<div v-for="(item, index) in upiList" :key="index" class="upi_C-item">
					...
					<div class="upi-body">
						...
						<!--						<div class="upi-body-id">{{$t('phoneN')}}: {{ item.mobileNo }}</div>-->
					</div>
				</div>
			</template>
			<template v-else>
...
import { useFastUpiKycOtp } from '@/hooks'
const stting = SettingStore()
const router = useRouter()
const route = useRoute()
const { openKycOtpByBid } = useFastUpiKycOtp()
 ...
 */
const onLoad = async () => {
	const res = await AwaitApiResult(GetWithdrawals({ withdrawid: 27 }))
	if (res) {
		const list = res.data?.withdrawalslist || []
		upiList.value = list.filter((item) => {
			if (!bankCode.value) return true
			return item.bankCode === bankCode.value
		})
	}
}
const backToWithdraw = (bid: number) => {
	router.replace({
		name: 'Withdraw',
		query: {
			bid,
...
const activeUpiClick = (item: any) => {
	if (!item.isKycOnline && stting.getNeedFastKycValidIsOpen) {
		void openKycOtpByBid(item, {
			source: 'fast-upi',
			onVerified: async () => {
				await onLoad()
				backToWithdraw(item.bid)
			},
		})
		return
	}
