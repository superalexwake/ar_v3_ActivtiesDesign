				...
				<ul>
					...
					<li>
						...
						<div class="moto_select">
							<span class="position-text">{{ item.playType == 'Num' ? formatBet(item.betContent) : changeTextSelect(item.betContent) }}</span>
						</div>
					</li>
					<li>
                        <span>
			...
			<div class="moto-note-result">
				<div class="lottery_reslut">
					...
					<div v-if="item.orderStatus !== 0" class="moto_result">
						<div class="list-inlineB">{{ item.openResult }}</div>
						<div class="list-inlineB" >
							{{ changeTextColor(Number(item.openResult)) }}
						</div>
						<div class="list-inlineB violet" v-if="item.openResult == 0 || item.openResult == 5">
							{{ $t('purpleColor') }}
						</div>
						<div class="list-inlineB" >
							{{ Number(item.openResult) > 4 ? $t('betBig') : $t('betSmall') }}
