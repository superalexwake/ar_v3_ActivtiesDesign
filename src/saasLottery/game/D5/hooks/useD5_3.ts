import {WritableComputedRef, inject, provide, ComputedRef, computed, reactive, Ref, ref, watch, onMounted} from "vue";
...
export function useD5(){
	...
	const getlotteryissue=async ()=>{
		await Promise.all([getGameInfo(),getIssue(true)])
	}
     ...
     * 倒计时定时器
     */
    watch(() => countdown.value.seconds,  (value) => {
    ...
    watch(() => issue.value, async (newVal) => {
        const data = await getOpenLottery(newVal)
        state.historyIssues = data?.list || [];
        /*开奖结果*/
        await getWinLossResults();
    })

    const useProvide=()=>{
        provide<D5_3State>(INJECT_KEY,{
            betScopes,
            betMultiples,
            betMultiple,
            amount,
...
export function useD5Context(){
    return inject<D5_3State|any>(INJECT_KEY,{} as D5_3State|any)
}
