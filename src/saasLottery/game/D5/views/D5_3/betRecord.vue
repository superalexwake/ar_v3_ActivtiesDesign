<template>
	<div class="d5-record">
		<NavBar left-arrow class="main" title="5D" @click-left="back" />
		<!-- 时段 tab -->
		<div class="d5-record__nav">
			<van-tabs v-model:active="active" @click-tab="onClickTab">
				<van-tab v-for="(item, index) in tabs" :key="index" :title="item.gameName" />
			</van-tabs>
		</div>
		<!-- 列表：直接复用 D5 现成的纯展示组件 -->
		<div class="d5-record__list">
			<div class="d5-record__card" v-if="mayrecord.length">
				<MayrecordList :mayrecord="mayrecord" :gameCode="tabs[active]?.gameCode" />
			</div>
			<div v-if="!mayrecord.length && !loading" class="d5-record__empty">
				<Empty />
			</div>
		</div>
		<!-- 翻页 -->
		<div v-if="mayrecord.length" class="d5-record__foot">
			<div class="d5-record__foot-prev" :class="{ disabled: pageNo <= 1 }" @click="pPage">
				<van-icon name="arrow-left" size="20" />
			</div>
			<div class="d5-record__foot-page">{{ pageNo }}/{{ totalPage }}</div>
			<div class="d5-record__foot-next" :class="{ disabled: pageNo >= totalPage }" @click="nPage">
...
import MayrecordList from '@/saasLottery/game/D5/components/D5_3/MayrecordList.vue'

const router = useRouter()
const { gameList, getGameList } = useGlobal()

// 本游戏类型前缀（用于从后端 gameList 里筛出本游戏的时段）
const GAME_PREFIX = 'D5'
...
.d5-record {
	...
	// 复用的 MayrecordList 无左右内缩/背景，仅在有数据时套一层卡片补齐（空状态不带背景，和其它游戏一致）
	&__card {
