<template>
	<div class="tf_C">
		<div class="tf_C-l1">
			<div class="tf_C-l1-l">
				<div class="tf_C-l1-no">{{ $t('gameRecordNum') }}</div>
				<div class="tf_C-l1-tip" @click="onSwitchIntroduce">
			...
			<div class="tf_C-l1-r" @click="goTrx">
				<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none">
					<path
		...
		<div class="tf_C-l3">
			<template v-for="(item, index) in prizeNumList" :key="index">
				<p v-if="result == item" :class="['num' + item, result == item && 'prize' + item]"></p>
				<div v-else :class="['num' + item, result == item && 'prize' + item]"></div>
			</template>
		</div>
		<!-- 玩法说明-->
		<van-popup @open="getIntroduce" v-model:show="introduceDialog" :close-on-click-overlay="false" round teleport="body">
...
const prizeNumList = computed(() => {
	if (!resultAll.value) return [0, 0, 0, 0, 0]
	let str = resultAll.value.substring(resultAll.value.length - 5).toUpperCase() //获取后5位
	if (/\d/.test(str)) {
		return [...str]
	} else {
		return [...resultAll.value.substring(resultAll.value.length - 5, resultAll.value.length - 10).toUpperCase()]
	}
})
...
.tf_C {
	...
	&-l3 {
		...
		@each $i in $list {
			.num#{$i} {
				background-image: url('@game/TrxWinGo/assets/trx2/img/num#{$i}.png');
			}

			.num#{$i}.prize#{$i}:last-of-type {
				background-image: url('@game/TrxWinGo/assets/trx2/img/prize#{$i}.png');
			}
		}
