<template>
	<div class="k3-record">
		<NavBar left-arrow class="main" title="K3" @click-left="back" />
		<!-- 时段 tab -->
		<div class="k3-record__nav">
			<van-tabs v-model:active="active" @click-tab="onClickTab">
				<van-tab v-for="(item, index) in tabs" :key="index" :title="item.gameName" />
			</van-tabs>
		</div>
		<!-- 列表 -->
		<div class="k3-record__list">
			<BetRecordList v-if="mayrecord.length" :mayrecord="mayrecord" />
			<div v-if="!mayrecord.length && !loading" class="k3-record__empty">
				<Empty />
			</div>
		</div>
		<!-- 翻页 -->
		<div v-if="mayrecord.length" class="k3-record__foot">
			<div class="k3-record__foot-prev" :class="{ disabled: pageNo <= 1 }" @click="pPage">
				<van-icon name="arrow-left" size="20" />
			</div>
			<div class="k3-record__foot-page">{{ pageNo }}/{{ totalPage }}</div>
			<div class="k3-record__foot-next" :class="{ disabled: pageNo >= totalPage }" @click="nPage">
...
import BetRecordList from '@/saasLottery/game/K3/components/k33/betRecordList.vue'

const router = useRouter()
const { gameList, getGameList } = useGlobal()

// 本游戏类型前缀（用于从后端 gameList 里筛出本游戏的时段）
const GAME_PREFIX = 'K3'
