...
import { computed, onMounted, provide, ref,onBeforeUnmount } from 'vue'
...
const { getWebData, currentGame } = useGlobalContext()
...
<template>
    <div class="videowingo">
        <LotteryInfo :showNav="false" @change-select-game="changeSelectGame" @setVoice="setVoice" :VoiceType="VoiceType" :countdown="countdown" />
        <WinGoInfo :handleRule="onSwitchIntroduce" :issue="issue" :numbers="winGoNumber5" :countdownTime="countdownTime" />
        <div class="Betting__C">
            <div v-show="showMark&&!closeGame&&!animationRoll" class="Betting__C-mark">
                <div v-for="i in secondsStr">{{ i }}</div>
                <!-- <div>{{ props.currentInfo.time4 || '0' }}</div>-->
            </div>
			<div v-if="closeGame"  class="Betting__C-mark closeGame">
				<p >{{$t('common.code_405')}}</p>
			</div>
			<div  v-if="animationRoll&&!closeGame">
				<transition
					name="slide-fade"
					mode="out-in"
				>
					<div :key="issue"  class="Betting__C-mark">
						<ul>
							<li> {{$t('issueTips',[issue])}}</li>
							<li >{{countdown.seconds===1?$t('betEnd'):$t('betStart')}}</li>
						</ul>
					</div>
				</transition>
        ...
        <div class="history">
            <div class="nav noScroll"  ref="navRef">
				<div class="nav-container noScroll" >
