...
import {INJECT_KEY, STORAGE_TOKEN, useGlobalContext,useWebSocket, useLottery, useStorage, useToast} from "@/saasLottery/hooks";
import {
    GetBetLimitRsp,
...
export function useWinGo3() {
	...
	const websocket=computed(()=>{
		const url=gameInfo.value?.webSocketUrl;
		if (!url) return '';
		return url+'/connect'+`?token=${localStore.get(STORAGE_TOKEN)}`
	})
   const {open,close:websocketClose}=useWebSocket(()=>websocket.value,{
	   autoReconnect: {
		   retries:10,
		   delay: 3000,
	   },
    ...
    const getWinLossResult = async () => {
        try {
			let list = [...mapBet.keys()].reverse();
			if (countdown.value.seconds>0&&countdown.value.seconds<=countdown.value.interval){
				list=list.filter((item)=>item!==issue.value);
			};
    ...
    const onBetting = async () => {
        ...
        loading.value = true;
        try {
            const { result } = await getVideoWinGoBet({
    ...
    // 播放音频
    const voicePlay = async (action = 1) => {
		const volumeShow=localStorage.getItem('volumeShow')||'';
		if (volumeShow=='2') return;
        const ttsAudio: any = document.getElementById(`voice${action}`);
        if (ttsAudio) {
			if (!(await canAutoPlay(ttsAudio.src)))return;
           try {
			   ttsAudio?.play();
		   }catch (e) {
